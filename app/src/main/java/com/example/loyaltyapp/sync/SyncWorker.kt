...
import com.example.loyaltyapp.core.session.AttendantCredentialGarbageCollector
 ...
 * notifying the attendant for each one, pulls down today's sales from the server so one recorded
 * on a different device signed into the same attendant account shows up here too (see
 * `SaleRepository.pullTodayFromServer`), and finally prunes any retained refresh token whose
 * attendant now has nothing left pending (see `AttendantCredentialGarbageCollector`).
...
class SyncWorker @AssistedInject constructor(
    ...
    private val credentialGarbageCollector: AttendantCredentialGarbageCollector
    ...
    override suspend fun doWork(): Result {
        return try {
            ...
            credentialGarbageCollector.pruneFullySynced()
            Result.success()
        } catch (e: Exception) {
            Result.retry()
        }
